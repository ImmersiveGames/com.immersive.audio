...
The package does not use singletons, service locators, `Resources.Load`, `DontDestroyOnLoad`, hidden framework bootstrap, or silent fallback.
...
`AudioListenerRuntimeHost` is an optional component for explicit listener setup. It ensures an `AudioListener` on its target or own GameObject and reports duplicate listeners.

Duplicate listeners are not destroyed by default. The default duplicate policy is `ReportOnly`; disabling duplicates requires explicit `DisableDuplicates` policy on the component.
...
- Expecting listener cleanup: duplicate listeners are reported by default, not destroyed.
