...
`AudioListenerRuntimeHost` is an optional Unity component. When present, it ensures an `AudioListener` on the configured target or on the host GameObject. It reports duplicate enabled listeners and does not destroy duplicate listeners.

The duplicate policy defaults to `ReportOnly`. Duplicates are disabled only when the component is explicitly configured with `DisableDuplicates`.
...
`AudioRuntimeHost` is an optional explicit Unity component. It receives `AudioDefaultsAsset`, may receive an explicit `PoolRuntimeHost`, composes `AudioSettingsService`, `AudioRoutingResolver`, `AudioGlobalSfxService`, and `AudioBgmService`, and exposes simple manual `PlaySfx`, `PlayBgm`, and `StopBgm` methods.
